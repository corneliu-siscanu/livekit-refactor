...
export function hasTransferOptions(config: AgentRuntimeConfig): boolean {
...
export function buildCompanySummary(config: AgentRuntimeConfig): string {
...
export function buildInstructions(config: AgentRuntimeConfig): string {
...
export async function appendRagContext(
  ...
  try {
    const { hits } = await ragService.search(query);
    if (hits.length === 0) {
      return;
    }
